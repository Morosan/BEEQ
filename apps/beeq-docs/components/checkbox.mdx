...
<CardGroup cols={2}>
  <Card title="Single checkbox">
    Use a single checkbox for an independent yes-or-no choice, such as subscribing to updates or accepting terms.
  </Card>
  <Card title="Checkbox group">
    Use a group when several related options can be selected together and none of them are mutually exclusive.
  </Card>
  <Card title="Nested selection">
    Use a parent checkbox when one control should affect a related set of child checkboxes.
  </Card>
  <Card title="Indeterminate state">
...
A checkbox is composed of the control itself and a visible text label. In grouped scenarios, a checkbox may also participate in a larger fieldset or parent-child relationship.

| Part | Element | Description |
|---|---|---|
| **1** | Control | The square selection indicator people toggle on or off |
| **2** | Check or indeterminate mark | The visual state shown when the checkbox is selected or partially selected |
| **3** | Label | The text that explains what the checkbox controls |
...
<CodeLivePreview code={`<bq-checkbox name="notifications" value="notifications" background-on-hover>
  ...
  <CodeGroup>
    ...
    <template>
      <BqCheckbox name="notifications" value="notifications" background-on-hover>
        Email me about account activity
      </BqCheckbox>
    </template>
    ```
...
<CodeLivePreview
  ...
  <script>
    (async () => {
      ...
      const checkboxGroup = previewRoot?.querySelector('#interests-group');
  ...
  <CodeGroup>
    ...
    const allCheckboxChange = (event) => {
      const interestCheckboxes = Array.from(
        document.querySelectorAll('#interests-group bq-checkbox[name="interest"]'),
      );
    ...
    const interestCheckboxChange = () => {
      const allInterestCheckbox = document.querySelector(
        '#interests-group bq-checkbox[name="all-interests"]',
      );
      if (!allInterestCheckbox) return;

      const interestCheckboxes = document.querySelectorAll(
        '#interests-group bq-checkbox[name="interest"]',
      );
      const onlyChecked = document.querySelectorAll(
        '#interests-group bq-checkbox[name="interest"][checked]',
      ).length;
...
- **Associate each checkbox with a visible label** — the text should explain the effect of the selected state without requiring extra context.
- **Group related choices semantically** — when multiple checkboxes form a set, provide a shared heading or fieldset label.
- **Preserve keyboard support** — people should be able to tab to each checkbox and toggle it with <kbd>Space</kbd>.
- **Use indeterminate carefully** — it should summarize a partial group selection, not replace clear child options.
- **Do not rely on color alone** — the checked mark, focus state, and label text should all help communicate meaning.
