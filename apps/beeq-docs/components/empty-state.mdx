...
- **Treat the default icon as decorative** — when the built-in thumbnail or a custom illustration does not add essential information, hide it from assistive technology with `aria-hidden="true"` or an empty `alt` value.
...
<Expandable title="View all CSS variables" defaultOpen={true}>
  ...
  |---|---|---|
  | None exposed | This component does not currently expose component-scoped CSS custom properties | — |
</Expandable>
