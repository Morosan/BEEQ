...
- `bq-date-picker` is form-associated and delegates focus, so it participates in native forms and moves focus into the input control when the component receives focus.
- The component emits `bqFocus`, `bqBlur`, `bqChange`, and `bqClear`, which gives you the hooks needed to announce validation, react to changes, and keep form state synchronized.
- Always provide a visible label through the `label` slot or an equivalent accessible name so assistive technologies can identify the purpose of the field.
- Selected, disabled, warning, and error states should never rely on color alone. Pair validation styling with clear text feedback and task-specific messages.
- Keep locale, first-day-of-week, and display formatting aligned with user expectations so typed input and visible values remain understandable.
