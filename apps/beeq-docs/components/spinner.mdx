...
- **Understand the built-in behavior**: the default loader includes a status element with an accessible loading label. If you replace it with the `icon` slot, test that the loading state is still understandable in context.
