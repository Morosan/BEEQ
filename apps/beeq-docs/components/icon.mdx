...
`bq-icon` keeps its structure intentionally small so it can be reused inside many higher-level components. The icon graphic sits inside a defined bounding area that controls its rendered footprint and alignment.

| Part | Element | Description |
|---|---|---|
| **1** | Icon | The visible icon graphic that communicates the action, object, or concept |
| **2** | Bounding area | The space that defines the icon footprint and keeps sizing and alignment consistent |
